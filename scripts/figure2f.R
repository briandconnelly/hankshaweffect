...
mutation_labels_log <- c(expression(paste(5, 'x', 10^{-7})),
                          expression(paste(5, 'x', 10^{-6})),
                          expression(bold(paste('5', 'x', '10'^{'-5'}))),
                          expression(paste(5, 'x', 10^{-4})),
                          expression(paste(5, 'x', 10^{-3})),
                          expression(paste(5, 'x', 10^{-2})),
...
g <- ggplotGrob(fig2f)
g <- gtable_add_grob(g, textGrob(expression(bold('F')), gp=gpar(col='black', fontsize=20), x=0, hjust=0, vjust=0.5), t=1, l=2)

png('../figures/Figure2f.png', width=6, height=3.708204, units='in', res=figure_dpi)
grid.newpage()
grid.draw(g)
dev.off()
