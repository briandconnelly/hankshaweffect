...
figs6b <- ggplot(data_figs6b, aes(x=Time, y=MeanProducerProportion)) +
    geom_vline(aes(xintercept=seq(from=0, to=max(data_figs6b$Time), by=change_freq)), color='grey80', size=0.3) +
...
png('../figures/FigureS6b.png', width=6, height=3.708204, units='in', res=figure_dpi)
grid.draw(g)
dev.off()
